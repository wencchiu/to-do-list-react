...
class App extends Component {
  ...
  editTaskContent (taskNum, content) {
    ...
    this.state.taskNumAndTasks.splice(
      index , 1, {taskNum: taskNum, taskContent:content.trim()});
    this.setState({
      taskNumAndTasks: this.state.taskNumAndTasks,
    })
  }
  ...
  moveTaskUp (taskNum) {
    ...
    if (index > 0) {
      this.state.taskNumAndTasks.splice(
        index-1 , 2, temp, this.state.taskNumAndTasks[index - 1]);
      this.setState({
        taskNumAndTasks: this.state.taskNumAndTasks,
      })
    }
  ...
  moveTaskDown (taskNum) {
    ...
    if (index < taskNumArr.length-1 ) {
      this.state.taskNumAndTasks.splice(
        index , 2, this.state.taskNumAndTasks[index + 1], temp);
      this.setState({
        taskNumAndTasks: this.state.taskNumAndTasks,
      })
    }
  ...
  render(){
    ...
    return (
      <div>
        ...
        <ListContainer taskNumAndTasks={this.state.taskNumAndTasks}
        deleteClicked={this.deleteClicked} modifyClicked={this.modifyClicked}
        editTaskContent={this.editTaskContent} moveTaskUp={this.moveTaskUp}
